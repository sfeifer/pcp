...
keySlotsInit(dict *config, void *events)
{
    ...
    */
    ...
    context->opts.connect_timeout = &connection_timeout;      // struct timeval*
    ...
    if ((context->slots.acc = keyClusterAsyncContextInit(&context->opts)) == NULL) {
        /* Coverity CID370635 */
    ...
    if (context->slots.acc->err) {
        pmNotifyErr(LOG_ERR, "%s: %s\n", "keySlotsInit", context->slots.acc->errstr);
	sdsfree(def_servers);
	return context;
    }
    ...
    sts = keyClusterSetOptionTimeout(&context->slots.acc->cc, command_timeout);
    if (sts != RESP_OK) {
	pmNotifyErr(LOG_ERR, "%s: failed to set command timeout: %s\n",
			"keySlotsInit", context->slots.acc->cc.errstr);
	return context;
    }
