...
newentry_search(char *buf)
{
    ...
    /*
     * Metric with pre-resolved indom: @ M name<TAB>indom<TAB>oneline
     * Generated by pmsearch_daily to avoid needing PMNS/PMCD lookup.
     */
    if (*p == 'M' && isspace((int)*(p + 1))) {
	char	*indom_str;
